...
namespace Qadra::Render {
  void GridPass::init(const QString &vertexSource, const QString &fragmentSource) {
    ...
    m_vao.attribute({.index = 0, .size = 2, .type = GL_DOUBLE, .relativeOffset = offsetof(Segment, from)});
    m_vao.attribute({.index = 1, .size = 2, .type = GL_DOUBLE, .relativeOffset = offsetof(Segment, to)});
    m_vao.attribute({.index = 2, .size = 4, .type = GL_FLOAT, .relativeOffset = offsetof(Segment, color)});
    m_vao.attribute({.index = 3, .size = 1, .type = GL_FLOAT, .relativeOffset = offsetof(Segment, lineWidthPixels)});
    m_vao.attribute({.index = 4, .size = 1, .type = GL_FLOAT, .relativeOffset = offsetof(Segment, antiAliasWidthPixels)});
  ...
  void GridPass::render(const Core::Camera &camera, const glm::vec2 &viewportSizePixels) const {
    ...
    for (const auto &[start, end, major]: lines) {
      const bool axis =
        (std::abs(start.x) <= 1e-12 && std::abs(end.x) <= 1e-12) ||
