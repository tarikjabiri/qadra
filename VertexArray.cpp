...
namespace Qadra::GL {
  ...
  void VertexArray::attribute(const Attribute &attr) const noexcept {
    ...
    if (attr.type == GL_DOUBLE) {
      glVertexArrayAttribLFormat(m_handle, attr.index, attr.size, attr.type, attr.relativeOffset);
    } else {
      glVertexArrayAttribFormat(m_handle, attr.index, attr.size, attr.type, attr.normalized, attr.relativeOffset);
    }
