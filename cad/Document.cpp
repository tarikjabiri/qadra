...
namespace Qadra::Cad
{
  ...
  {
    auto handle = next ();
    auto entity = std::make_unique<Entity::Arc> ( handle, ++m_renderKeySeed, record );
    m_spatialIndex.insert ( handle, entity->bbox () );
    m_entities.emplace ( handle, std::move ( entity ) );
    m_dirtyFrom = std::min ( m_dirtyFrom, static_cast<std::size_t> ( m_drawOrder.size () ) );
    m_drawOrder.append ( handle );
  ...
  {
    auto handle = next ();
    auto entity = std::make_unique<Entity::Circle> ( handle, ++m_renderKeySeed, record );
    m_spatialIndex.insert ( handle, entity->bbox () );
    m_entities.emplace ( handle, std::move ( entity ) );
    m_dirtyFrom = std::min ( m_dirtyFrom, static_cast<std::size_t> ( m_drawOrder.size () ) );
    m_drawOrder.append ( handle );
  ...
  {
    auto handle = next ();
    auto entity = std::make_unique<Entity::LWPolyline> ( handle, ++m_renderKeySeed, record );
    m_spatialIndex.insert ( handle, entity->bbox () );
    m_entities.emplace ( handle, std::move ( entity ) );
    m_dirtyFrom = std::min ( m_dirtyFrom, static_cast<std::size_t> ( m_drawOrder.size () ) );
    m_drawOrder.append ( handle );
