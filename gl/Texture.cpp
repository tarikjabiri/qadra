...
namespace Qadra::GL
{
  ...
  void Texture::upload ( const GLint x, const GLint y, const GLsizei width, const GLsizei height,
                         const GLenum format, const GLenum type, const GLvoid *data ) const
  {
    glTextureSubImage2D ( m_handle, 0, x, y, width, height, format, type, data );
  }

  void Texture::bind ( const GLuint unit ) const { glBindTextureUnit ( unit, m_handle ); }
