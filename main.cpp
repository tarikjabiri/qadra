...
int main ( int argc, char *argv[] )
{
  ...
  const QString splashPath =
      isDarkTheme ? ":/assets/qadra-splash-dark.svg" : ":/assets/qadra-splash-light.svg";
  ...
  bar.setStyleSheet ( isDarkTheme
                          ? "QProgressBar { background: rgba(148, 163, 184, 0.22); border: none; "
                            "border-radius: 2px; }"
                            "QProgressBar::chunk { background: #38BDF8; border-radius: 2px; }"
                          : "QProgressBar { background: rgba(14, 116, 144, 0.16); border: none; "
                            "border-radius: 2px; }"
  ...
  const int remainingSplashDurationMilliseconds = std::max (
      0, minimumSplashDurationMilliseconds - static_cast<int> ( splashTimer.elapsed () ) );

  QTimer::singleShot ( remainingSplashDurationMilliseconds,
                       [&w, &splash] ()
                       {
                         w.show ();
                         splash.close ();
