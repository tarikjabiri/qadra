...
namespace Qadra::Ui
{
  DrawMenu::DrawMenu ( QWidget *parent ) : QMenu ( parent )
  {
    setObjectName ( "DrawMenu" );
    setTitle ( tr ( "&Draw" ) );
    ...
    m_polylineAction = addAction ( QIcon ( ":/assets/draw-lwpolyline.svg" ), tr ( "Polyline" ) );
    ...
    m_actionGroup->addAction ( m_polylineAction );
    ...
    bindAction ( m_polylineAction, Tool::ToolKind::Polyline );
  ...
  DrawMenu::~DrawMenu () = default;

  void DrawMenu::setSelectedToolKind ( const Tool::ToolKind kind )
  {
    if ( selectedToolKind () == kind ) return;

    const QSignalBlocker lineBlocker ( m_lineAction );
    const QSignalBlocker polylineBlocker ( m_polylineAction );
    ...
    if ( m_polylineAction ) m_polylineAction->setChecked ( kind == Tool::ToolKind::Polyline );
  ...
  Tool::ToolKind DrawMenu::selectedToolKind () const noexcept
  {
    if ( m_lineAction && m_lineAction->isChecked () ) return Tool::ToolKind::Line;
    if ( m_polylineAction && m_polylineAction->isChecked () ) return Tool::ToolKind::Polyline;
